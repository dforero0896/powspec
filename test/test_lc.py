...
import pandas as pd
dat_fname = "/global/cfs/cdirs/desi/mocks/UNIT/HOD_Shadab/multiple_snapshot_lightcone/UNIT_lightcone_multibox_ELG_footprint_nz_NGC.dat"
ran_fname = "/global/cfs/cdirs/desi/mocks/UNIT/HOD_Shadab/multiple_snapshot_lightcone/UNIT_lightcone_multibox_ELG_footprint_nz_1xdata_5.ran_NGC.dat"
...
pk = compute_cross_lc(data[:,0], data[:,1], data[:,2], wdata, fkp_data, data[:,3],
                    ...
                    rand[:,0], rand[:,1], rand[:,2], wrand, fkp_rand, rand[:,3],
                    conf = "test/powspec_lc.conf",
                    auto = ["test/lc_auto_test_1.powspec","test/lc_auto_test_1.powspec"],
                    cross = "test/lc_cross_test.powspec")
for i in range(3):
    for j in range(2):
...
pk = compute_auto_lc(data[:,0], data[:,1], data[:,2], wdata, fkp_data, data[:,3],
                    rand[:,0], rand[:,1], rand[:,2], wrand, fkp_rand, rand[:,3],
                    conf = "test/powspec_lc.conf",
                    auto = "test/lc_auto_test.powspec")
